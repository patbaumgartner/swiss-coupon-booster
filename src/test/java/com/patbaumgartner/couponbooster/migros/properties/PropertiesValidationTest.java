...
class PropertiesValidationTest {
	...
	void shouldValidatePlaywrightProperties() {
		// Valid properties
		var validProps = new MigrosPlaywrightProperties("https://login.migros.ch/",
				"https://login.migros.ch/login/password", 50, 100, true, List.of("--no-sandbox"), 5000);
		var violations = validator.validate(validProps);
		assertThat(violations).isEmpty();

		// Invalid timeout
		var invalidTimeout = new MigrosPlaywrightProperties("https://login.migros.ch/",
