...
class MigrosBrowserFactoryTest {

	@Mock
	private MigrosPlaywrightProperties playwrightProperties;

	@InjectMocks
	private MigrosBrowserFactory browserFactory;
	...
	void constructor_WithValidProperties_ShouldCreateFactory() {
		...
		// When
		var factory = new MigrosBrowserFactory(properties);
	...
	private MigrosPlaywrightProperties createValidProperties() {
		return new MigrosPlaywrightProperties("https://login.example.com", "https://login.example.com/password", 50,
				100, true, List.of("--no-sandbox"), 10000);
	}
