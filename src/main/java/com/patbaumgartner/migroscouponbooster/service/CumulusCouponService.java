...
import com.patbaumgartner.migroscouponbooster.config.ApplicationConstants;
import com.patbaumgartner.migroscouponbooster.exception.CouponBoosterException;
import com.patbaumgartner.migroscouponbooster.model.CouponActivationResult;
import com.patbaumgartner.migroscouponbooster.model.CouponDetail;
import com.patbaumgartner.migroscouponbooster.model.CouponInfo;
import com.patbaumgartner.migroscouponbooster.properties.CumulusProperties;
...
import static com.patbaumgartner.migroscouponbooster.config.ApplicationConstants.CookieNames.*;
...
public final class CumulusCouponService implements CouponService {
	...
	public CumulusCouponService(final RestClient.Builder restClientBuilder, final CumulusProperties configuration) {
		...
		this.apiClient = restClientBuilder.baseUrl(configuration.urls().baseUrl())
			.defaultHeader(HttpHeaders.USER_AGENT, ApplicationConstants.ApplicationInfo.DEFAULT_USER_AGENT)
			.build();
	}

	public CouponActivationResult activateAllAvailableCoupons(final List<Cookie> sessionCookies) {
		...
		try {
			...
			var availableCoupons = fetchAvailableCoupons(filteredCookies);
			return processCouponActivations(availableCoupons, filteredCookies);
		}
	...
	private List<CouponInfo> fetchAvailableCoupons(final List<Cookie> sessionCookies) {
		...
		try {
			...
			CouponsResponse apiResponse = this.apiClient.get()
				...
				.header(HttpHeaders.REFERER, this.configuration.urls().couponsReferer())
				.header(ApplicationConstants.HttpHeaders.CSRF_TOKEN_HEADER, extractCsrfToken(sessionCookies))
				.retrieve()
			...
			return Stream.of(apiResponse.available(), apiResponse.preview())
	...
	private CouponActivationResult processCouponActivations(final List<CouponInfo> allCoupons,
		...
		var activationResults = inactiveCoupons.stream()
			.map(coupon -> activateSingleCoupon(coupon.id(), sessionCookies))
			.toList();
	...
	private CouponDetail activateSingleCoupon(final String couponId, final List<Cookie> sessionCookies) {
		try {
			...
			var activationRequest = this.apiClient.post()
				...
				.header(HttpHeaders.COOKIE, cookieHeader)
				.header(HttpHeaders.REFERER, this.configuration.urls().couponsEndpoint());

			if (csrfToken != null && !csrfToken.isBlank()) {
				activationRequest = activationRequest.header(ApplicationConstants.HttpHeaders.CSRF_TOKEN_HEADER,
						csrfToken);
			}
			else {
